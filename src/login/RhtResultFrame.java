...
public class RhtResultFrame {
    ...
    public RhtResultFrame(String allReuslt) {
        aptframe = new JFrame();
        aptdlg = new JDialog(aptframe, "연립다세대 검색 결과", Dialog.ModalityType.APPLICATION_MODAL);
        aptdlg.getContentPane().setLayout(new GridBagLayout());
        gridBagConstraints = new GridBagConstraints();

        curPage = 1;
        String[] temp = allReuslt.split("\\n+");
        resultList = new StringBuilder[temp.length / 8];
        ...
        for (int i = 1; i < resultList.length; i++) {
            ...
            for (int j = 0; j < 8; j++) {
                resultList[i - 1].append(temp[i * 8 + j]);
    ...
    private void newPage(int pageNum) {
        ...
        for (int i = 0; i < result.length; i++) {
            ...
            if (resultList[i + (curPage - 1) * 4] != null) {
                ...
                result[i].setPreferredSize(new Dimension(300, 100));
